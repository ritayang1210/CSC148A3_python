...
class TippyState(GameState):
    ...
    def apply_move(self, move):
        ...
        if move in self.possible_next_moves():
            ...
            next_state = TippyState(self.opponent(), grid_num=self.grid_num,
                                    _grid=next_state_grid)
            return next_state
        else:
            return None
    ...
    def get_move(self):
        ...
        TippyMove((4, 3))
        '''
        i = input('choose to place at row: ')
        while not i.isnumeric():
            i = input('choose to place at row: ')
        j = input('choose to place at column: ')
        while not j.isnumeric():
            j = input('choose to place at column: ')

        return TippyMove((int(i), int(j)))
    ...
    def winner_check(self, L):
        ...
        Lz = []
        for ele in L:
            if (ele + self.grid_num) in L and (ele % self.grid_num != 0
               and ele % self.grid_num != 1):
                Lz.append([ele, (ele+self.grid_num)])
        for ele in Lz:
            if (((ele[0] - 1) in L) and ((
                ele[1] + 1) in L)) or (((
                    ele[0] + 1) in L) and ((ele[1] - 1) in L)):
                return True
        Ls = []
        for ele in L:
                if ((ele + 1) in L) and (ele % self.grid_num != 0):
                    Ls.append([ele, (ele+1)])
        for ele in Ls:
            if (((ele[0] - self.grid_num) in L) and ((
                ele[1] + self.grid_num) in L)) or (((
                    ele[0] + self.grid_num) in L) and ((
                        ele[1] - self.grid_num) in L)):
                return True
        else:
            return False
    ...
    def winner(self, player):
        ...
        if self.winner_check(self.currentXO(player)
                             ) and self.opponent() == player:
            return True
        else:
            return False
